...
This module provides utility functions for MLflow integration including model
registration, loading from registry, metrics logging, and run management for
the network intrusion detection project.
"""
...
def register_model(
    ...
    try:
        ...
        logger.info(
            f"Model {model_name} v{model_version.version} registered and transitioned to {stage}"
        )
...
def get_model_from_registry(model_name: str, stage: str = "Production") -> Any:
    ...
    try:
        ...
        model = mlflow_lightgbm.load_model(model_uri)
